...
//	MYERROR( "USB3Analyzer : myOpenResources" );
	...
	for( pos=0 ; pos < max_libs ; pos++ )
	{
		...
		switch( OpenList[pos].os_Type )
		{
			case OT_Library:
			{
				...
				if ( ! base )
				{
					printf( "USB3Analyzer : Error missing base pointer" );
					goto bailout;
				}
...
//					MYINFO( "USB3Analyzer : Opening library '%s' v%lu", name, vers );

					ptr = OpenLibrary( name, vers );

					if ( ! ptr )
					{
						printf( "USB3Analyzer : Error opening library '%s' v%u", name, vers );
						goto bailout;
					}
			...
			case OT_Interface:
			{
				...
				if ( ! ifc )
				{
					printf( "USB3Analyzer : Error missing interface pointer" );
					goto bailout;
				}
...
//					MYINFO( "USB3Analyzer : Getting interface pos #%lu", pos );

					ptr = GetInterface( *base, name, vers, NULL );

					if ( ! ptr )
					{
						printf( "USB3Analyzer : Error getting interface pos #%u", pos );
						goto bailout;
					}
			...
			case OT_Class:
			{
				...
				if ( ! base )
				{
					printf( "USB3Analyzer : Error missing class pointer" );
					goto bailout;
				}
				...
				if ( *base == NULL )
				{
					...
					if ( ! IIntuition )
					{
						printf( "USB3Analyzer : Error need Intuition" );
						goto bailout;
					}
...
//					MYINFO( "USB3Analyzer : Opening class '%s'", name );

					ptr = OpenClass( name, vers, cls );

					if ( ! ptr )
					{
						printf( "USB3Analyzer : Error opening class '%s' pos #%u", name, pos );
						goto bailout;
					}
			...
			default:
			{
				printf( "USB3Analyzer : Unknown Resource type (%u)", OpenList[pos].os_Type );
				goto bailout;
			}
		}
	}
...
//	MYERROR( "USB3Analyzer : myCloseResources" );
	...
	{
		pos--;
//		MYERROR( "USB3Analyzer : Processing : %s", OpenList[pos].os_STR );
