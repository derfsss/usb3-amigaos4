...
// TRB control field -- Endpoint ID (bits 16-20, in command and event TRBs)
#define XHCI_TRB_SET_EP(x)			(((x) & 0x1FU) << 16)
#define XHCI_TRB_GET_EP(x)			(((x) >> 16) & 0x1FU)

// Transfer Event control field -- Event Data flag (bit 2). When set, the
// event was generated by an Event Data TRB: the event's param echoes the
// Event Data TRB's param, and the event's transfer-length field holds the
// EDTLA (total bytes transferred for the TD), NOT a residual.
#define XHCI_TRB_EVENT_ED			0x00000004UL
